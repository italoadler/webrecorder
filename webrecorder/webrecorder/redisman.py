...
class CollManagerMixin(object):
    ...
    def get_collections(self, user, include_recs=False):
        key_pattern = self.coll_info_key.format(user=user, coll='*')
        ...
        # if not owner or superuser, filter out private collections
        if not self.is_owner(user) and not self.is_superuser():
